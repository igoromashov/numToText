...
function numToText(num, set = 1) {
  ...
  const degreeName = (prePrevious, previous, current, hundredDegree) => {
    ...
    // тогда назначаем имя класса в родительном падеже мн. числе (соответствует кол-ву >= 5)
  ...
  // обработка дополнительного параметра функции (настройка выдачи валюты и копеек)
  switch (set) {
    case 0: { // возвращаем результат соединения элементов массива в строку через пробел
      return textInteger.join(" ");
    };
    case 1: { // возвращаем результат соединения элементов массива в строку через пробел, валюту и копейки
